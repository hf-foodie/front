...
import { TComment as TCommentArray } from '@/data/comment.server'

type TComment = TCommentArray[number]
...
const CommentCard: FC<Props> = ({ className, comment }) => {
  const { id, name, text, like, date } = comment
  ...
  return (
    <>
      <div className={clsx(`comment-card flex ${className}`)}>
        <div className="flex size-8 shrink-0 items-center justify-center rounded-full bg-neutral-100 text-neutral-500 dark:bg-neutral-800 dark:text-neutral-400">
        ...
        <div className="ms-2 flex grow flex-col rounded-xl border border-neutral-200 p-4 text-sm sm:ms-3 sm:text-base dark:border-neutral-700">
          ...
          {isReplying ? (
            ...
            <div className="flex items-center gap-2">
              <CommentLikeBtn likeCount={like} isLiked={false} postId={id} />
