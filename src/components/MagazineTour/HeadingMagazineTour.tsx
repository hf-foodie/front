...
export function HeadingMagazineTour({ className, level = 2, dimHeading, children, ...props }: HeadingProps) {
  ...
  return (
    ...
    >
      {children}
      {dimHeading && '. '}
      {dimHeading && <span className="text-neutral-500 dark:text-neutral-500">{dimHeading}</span>}
    </Element>
  )
}
...
export default function HeadingWithSub({
  ...
  return (
    <div className={clsx(className, 'relative mb-12', isCenter && 'mx-auto w-full text-center text-pretty')}>
      <HeadingMagazineTour level={level} {...props}>
        {children}
      </HeadingMagazineTour>
