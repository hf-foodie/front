...
import CategoryMagazineTouristAttractionBadgeList
    from "@/components/SectionMagazineTour/CategoryMagazineTouristAttractionBadgeList";

interface Props {
  className?: string
  touristAttraction: TTouristAttraction
  galleryType?: 1 | 2
}

const CardMagazineTour: FC<Props> = ({ className, touristAttraction, galleryType = 1 }) => {
  const { title, handle, categories, postType, galleryImgs, author, date, readingTime, bookmarked } = touristAttraction
    ...
    >
      <div className="relative aspect-square w-full overflow-hidden rounded-3xl sm:aspect-16/9">
        {postType !== 'gallery' || !galleryImgs?.length ? (
          <PostFeaturedMedia post={touristAttraction} isHover={isHover} />
        ) : galleryType === 1 ? (
          renderGallery()
        ) : (
          renderGallery2()
      ...
      <div className="absolute inset-x-3 top-3 flex items-start justify-between space-x-4">
        <CategoryMagazineTouristAttractionBadgeList categories={categories} />
...
export default CardMagazineTour
